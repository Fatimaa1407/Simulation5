...
using Simulation6.Utilities.Images;
...
namespace Simulation6.Areas.Admin.Controllers
{
    ...
    public class MemberController : Controller
    {
        ...
        public MemberController(IWebHostEnvironment env, AppDbContext db)
        {
            _env = env;
            _db = db;
        }
        ...
        {
            ViewBag.Positions = await _db.Positions.ToListAsync();
            Member oldMember = await _db.Members.FirstOrDefaultAsync(m=>m.Id == memberVM.Id);
            if (!ModelState.IsValid) return View(memberVM);
            if (memberVM.ImageFile is null)
            {
                ModelState.AddModelError("ImageFile", "File is required");
